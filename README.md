...
export const dynatraceConfig = {
  // Replace with your actual Dynatrace RUM script URL
...
1. Navigate to Dynatrace → **Applications & Microservices** → **Web applications**
...
### 3. Update `app.vue` (Frontend RUM Integration)

Update your `app.vue` file to load the Dynatrace RUM script from your configuration file:
...
### 5. Test PathReplace (if configured)
...
### Issue: RUM script not loading

**Possible causes:**
1. `dynatrace.config.ts` file not created
2. Invalid script URL in configuration
3. `enabled` is set to `false`

**Solution:**
```bash
# Verify config file exists
ls -la dynatrace.config.ts

# Check browser console for warnings:
# "Dynatrace config not found. Copy dynatrace.config.example.ts to dynatrace.config.ts"

# Verify script URL in dynatrace.config.ts
cat dynatrace.config.ts

# Ensure scriptUrl is not the placeholder value
# Should NOT be: 'YOUR_DYNATRACE_RUM_SCRIPT_URL'
```
...
4. RUM script not loaded (see above)
